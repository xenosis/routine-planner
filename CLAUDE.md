...
- Paper TextInput label 딜레이 버그: 값이 있는 필드를 열었을 때 label이 ~1초 뒤에 올라오는 현상
  - 근본 원인: Android에서 parent `opacity:0`이면 `onLayout`이 `{width:0, height:0}`으로 fire → Paper 내부 `measured=false` → label invisible
  - 해결: **`labelsReady` 패턴** — `requestAnimationFrame` 첫 프레임에 `value=""`로 렌더 → label `opacity:1` → `onLayout`이 실제 치수로 fire → `measured=true` → 다음 프레임에 실제 값 세팅 시 label 즉시 올라옴
    ```tsx
    ...
    ```
  - 전제 조건: 부모에서 **조건부 렌더링** (`{visible && <Screen />}`) + 자식에서 **`useState(() => prop?.value ?? '')`** 초기값 함수 사용
  - `AddScheduleScreen`은 `Portal` 기반 전체화면으로 구현 (ScheduleScreen에서 `{modalVisible && <AddScheduleScreen />}`)
...
#### 성과탭 주간 차트
- `quotaMetForWeek`: 이번 주 quota를 달성한 weekly_count 루틴 ID 집합
  - 각 날짜의 분모(`getScheduledCountForDate`)에서 해당 루틴 제외 → 100% 초과 방지
  - 예: 주 2회 루틴을 월·화에 완료 → 수~일의 분모에서 제외 → 해당 날 100% 유지

#### 성과탭 월간 달력
- `buildMarkedDates`: `getRoutineCompletionsInRange(startDate, endDate)` — (routineId, date) 쌍 전체 조회
- 날짜별 `quotaMetBeforeThisDay` 계산: 해당 날 **이전에** 이미 quota를 달성한 weekly_count 루틴 집합
  - 해당 날의 완료·예정 분모 모두에서 제외 → dot 색상(전체/일부/미완료) 정확도 개선
- `getWeekStart(dateStr)`: 날짜 문자열의 월요일(주 시작일) 반환 — 주별 누적 계산 기준
